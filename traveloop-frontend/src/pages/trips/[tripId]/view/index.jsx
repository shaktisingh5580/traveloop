          ...
          <div className="space-y-8">
            ...
            <div className="bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/40">
              ...
              <div className="space-y-4">
                {(trip?.members?.length > 0 ? trip.members : [{ id: 1, name: trip?.user?.full_name || "Host", role: "host" }]).map((member, m) => (
                  <div key={m} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-slate-100 overflow-hidden border-2 border-white shadow-sm">
                        <img src={member.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(member.email || member.name || m)}`} alt="Member" className="w-full h-full object-cover" />
                      </div>
                      <div>
                        <p className="text-sm font-bold text-slate-800">{member.full_name || member.name || "Traveler"} {member.role === "host" ? "(Host)" : ""}</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{member.role || "Editor"}</p>
                      </div>
                    </div>
              ...
              <div className="pt-4 border-t border-slate-100">
                 ...
                 <div className="flex -space-x-3">
                    <div className="w-10 h-10 rounded-full border-2 border-white bg-slate-100 overflow-hidden shadow-sm">
                      <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(trip?.user?.email || 'host')}`} alt="Host" className="w-full h-full object-cover" />
                    </div>
